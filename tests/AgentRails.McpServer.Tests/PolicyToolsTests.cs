...
public class PolicyToolsTests
{
    ...
    public async Task ListPolicies_ReturnsBody()
    {
        ...
        var tools = new PolicyTools(new FakeHttpClientFactory(handler));

        var result = await tools.list_policies();
    ...
    public async Task CreatePolicy_PostsAllFields()
    {
        ...
        var result = await tools.create_policy(
            name: "Strict",
            requiresApproval: true,
            description: "High-value policy",
            maxTransactionAmount: 500m,
    ...
    public async Task CreatePolicy_OmitsOptionalFields_WhenNull()
    {
        ...
        await tools.create_policy(name: "Basic", requiresApproval: false);
